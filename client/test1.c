...
int main( void )
{
  ...
  if ( ev3_search_tacho_plugged_in(left_wheel_port, 0, &sn_left, 0 )) {
    ...
    printf( "LEGO_EV3_M_MOTOR (left) is found, run for 5 sec...\n" );
  ...
  if ( ev3_search_tacho_plugged_in(right_wheel_port, 0, &sn_right, 0 )) {
    ...
    printf( "LEGO_EV3_M_MOTOR (right) is found, run for 5 sec...\n" );
  ...
  t = (float) (count_per_rot * abs(value) )/(current_speed * PI * WHEEL_DIAM) ;
  printf("Tachos are going to run for %f seconds \n", t);
